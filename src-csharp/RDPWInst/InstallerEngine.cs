...
internal sealed class InstallerEngine
{
    ...
    // The private copy is the default update source. An environment override
    // keeps enterprise/offline deployments able to point at an internal mirror.
    private static string ReleaseBaseUrl =>
        (Environment.GetEnvironmentVariable("RDPWRAP_RELEASE_BASE_URL")
         ?? "https://github.com/dipsx/rdpwrap-private/releases/latest/download/")
        .TrimEnd('/') + "/";
    ...
    private void ExtractFiles()
    {
        ...
        // ── Core DLL ──
        var dllRes = ArchHelper.IsArm64
            ? "RDPWInst.Resources.rdpwarm64.dll"
            : ArchHelper.Is64Bit
    ...
    /// <summary>
    /// If the running termsrv.dll version is absent from rdpwrap.ini, extracts
    /// the pinned OffsetFinder + Zydis resources embedded by the official build
    /// and runs the finder to append generated offsets. We deliberately do not
    /// execute binaries downloaded at install time.
    /// </summary>
    private void TryAutoGenerateOffsets()
    {
        ...
        if (ArchHelper.IsArm64)
        {
            Console.Error.WriteLine("[-] Automatic offset generation is not available for ARM64.");
            Console.Error.WriteLine("[!] Generate and add an ARM64 configuration manually.");
            return;
        }

        var archSuffix = ArchHelper.Is64Bit ? "_x64" : "_x86";
        var tempDir    = Path.Combine(
            Path.GetTempPath(), "rdpwrapoffset-" + Guid.NewGuid().ToString("N"));
        ...
        {
            var asm = Assembly.GetExecutingAssembly();
            var exeResource = $"RDPWInst.Resources.RDPWrapOffsetFinder{archSuffix}.exe";
            var dllResource = $"RDPWInst.Resources.Zydis{archSuffix}.dll";

            if (!ResourceHelper.ExtractToDisk(exeResource, exePath, asm) ||
                !ResourceHelper.ExtractToDisk(dllResource, dllPath, asm))
            {
                Console.Error.WriteLine("[-] Offset-generation tools are not embedded in this build.");
                Console.Error.WriteLine("[!] Use the official release package or add the tools during the build.");
                return;
            }
            ...
            bool ran = ProcessHelper.ExecWait($"\"{sysCmd}\" /c \"\"{exePath}\" >> \"{iniPath}\"\"");

            if (!ran)
            {
                Console.Error.WriteLine("[-] Offset finder failed to execute successfully.");
                return;
            }
        ...
        {
            // Clean up temporary tool files even when extraction or execution fails.
